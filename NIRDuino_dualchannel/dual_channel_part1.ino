...
// SET YOUR DESIRED LED COUNT HERE
#define USER_CONFIGURED_LED_COUNT  2

const int NUM_SOURCES = USER_CONFIGURED_LED_COUNT + 1; // LEDs + 1 dark cycle
...
// We will fill the specific values in the init function to be safe.
...
void printLEDIntensities() {
    ...
    for (int i = 0; i < NUM_LEDS; i++) {
        Serial.printf("LED %d: %d", i, ledIntensities[i]);
...
void initDefaultLEDIntensities(void)
{
    ...
    int defaultIntensities[NUM_SOURCES-1] = {100, 50};
...
static float read_detector_voltage(uint8_t detector) {
    ...
    // --- MAPPING LOGIC (FIXED) ---
    // This now matches the "jumbled" order from the adc_sampling_task
    // Detector 1 -> AIN1
    // ...
    // Detector 7 -> AIN7
    // Detector 8 -> AIN0
    ...
    if (detector <= 8) {
        ...
        uint8_t remapped_channel;
        if (detector == 8) {
            remapped_channel = 0; // Detector 8 maps to AIN0
        } else {
            remapped_channel = detector; // Detector 1 maps to AIN1, 2 to AIN2, etc.
        }
        
        // MUX codes: {AIN0=0x08, AIN1=0x18, ... AIN7=0x78}
        adc_channel = (remapped_channel << 4) | 0x08;
        ...
        uint8_t remapped_channel;
        uint8_t base_detector = detector - 8; // Map 9-16 to 1-8
        
        if (base_detector == 8) {
            remapped_channel = 0; // Detector 16 maps to AIN0
        } else {
            remapped_channel = base_detector; // Detector 9 maps to AIN1, 10 to AIN2, etc.
        }
...
static void adc_sampling_task(void* arg) {
    ...
    while (1) {
        ...
        if (xSemaphoreTake(adc_semaphore, portMAX_DELAY) == pdTRUE) {
            ...
            if (sourceNumber == (NUM_SOURCES - 1)) { // After processing source 32 (the 33rd cycle)
...
void ble_broadcast_task(void *pvParameters) {
    ...
    // 1. Calculate how many integers we actually have
    // 3 Sources (2 LEDs + 1 Dark) * 17 slots per source = 51 integers.
    const int TOTAL_DATA_INTS = NUM_SOURCES * SLOTS_PER_SOURCE;
    
    // 2. Create a buffer big enough for Data + 1 Header byte
    int32_t packetBuffer[TOTAL_DATA_INTS + 1];
    ...
    while(1) {
        if(xSemaphoreTake(ble_semaphore, portMAX_DELAY) == pdTRUE) {
            if (readDataBool && deviceConnected) {
                ...
                // 1. Add the Header (Packet ID = 1)
                packetBuffer[0] = 1;

                // 2. Direct Chunk Copy (Memcpy)
                // We copy ALL the data from dataPacketToSend into our buffer at once.
                // No loops, just one fast memory move.
                memcpy(&packetBuffer[1], &dataPacketToSend[0], TOTAL_DATA_INTS * sizeof(int32_t));

                // 3. Send the single chunk
                // Size is (51 data + 1 header) * 4 bytes
                sendDataViaBLE(pDataCharacteristic, (uint8_t*)packetBuffer, (TOTAL_DATA_INTS + 1) * sizeof(int32_t));
                
                // Serial.println("Sent single data packet.");
            }
        }
    }
}
